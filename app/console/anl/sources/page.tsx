...
type GroupBy = 'referrer' | 'utm_source' | 'utm_medium' | 'utm_campaign';
...
export default function Page() {
  ...
  const [groupBy, setGroupBy] = useState<GroupBy>('referrer');

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/analytics/data?tenant_id=' + unit.id + '&range=' + range + '&view=sources');
  ...
  const grouped = useMemo(() => {
    ...
    rows.forEach(r => {
      let key: string;
      if (groupBy === 'referrer') {
        key = extractDomain(r.referrer);
      } else {
        const page = r.page || '';
        try {
          const url = new URL(page, 'http://x');
          key = url.searchParams.get(groupBy) || 'None';
        } catch { key = 'None'; }
      }
      map[key] = (map[key] || 0) + 1;
    });
  ...
  return (
    <div>
      <ScreenHeader title="Sources" description="Traffic sources by referrer domain and UTM parameters" />
      <div className="mb-5 flex flex-wrap items-center gap-2">
        ...
        <div className="flex gap-1 ml-4 border-l border-slate-200 pl-4">
          {(['referrer', 'utm_source', 'utm_medium', 'utm_campaign'] as GroupBy[]).map(g => (
            <button key={g} onClick={() => setGroupBy(g)}
              className={'rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ' + (groupBy === g ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200')}>
              {g === 'referrer' ? 'Referrer' : g.replace('utm_', '').charAt(0).toUpperCase() + g.replace('utm_', '').slice(1)}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-xs font-semibold uppercase text-slate-400">
              <tr>
                <th className="px-5 py-3 text-left">{groupBy === 'referrer' ? 'Domain' : groupBy.replace('_', ' ')}</th>
                ...
                return (
                  <tr key={key} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-2.5">
                      <div className="h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-orange-500" style={{ width: `${Math.round((count / max) * 100)}%` }} /></div>
                    </td>
