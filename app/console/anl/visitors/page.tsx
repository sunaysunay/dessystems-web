...
export default function Page() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/analytics/data?tenant_id=' + unit.id + '&range=' + range + '&view=visitors');
  ...
  const grouped = useMemo(() => {
    ...
    rows.forEach(r => { const k = r['session_id'] || 'Unknown'; map[k] = (map[k] || 0) + 1; });
  ...
  return (
    <div>
      <ScreenHeader title="Visitors" description="Visitor and session breakdown" />
