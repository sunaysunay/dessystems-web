...
const columns: Column<PackOrder>[] = [
  ...
  { key: 'status', header: 'Status', width: '120px', render: (r) => <StatusBadge status={r.fulfilment_status ?? 'picking'} />, value: (r) => r.fulfilment_status ?? '' },
...
export default function SH081Page() {
  const [orders, setOrders] = useState<PackOrder[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/bop/shop/orders?status=picking').then(r => r.json()).then(d => {
  ...
  return (
    <div className="p-6 space-y-4">
      <ScreenHeader />
      <DataGrid columns={columns} rows={orders} rowKey={(r) => r.id} loading={loading} emptyMessage="No orders being packed" />
    </div>
  );
}
