...
export default function Page() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/analytics/data?tenant_id=' + unit.id + '&range=' + range + '&view=listing_views');
  ...
  const grouped = useMemo(() => {
    ...
    rows.forEach(r => { const k = r['listing_title'] || 'Unknown'; map[k] = (map[k] || 0) + 1; });
  ...
  return (
    <div>
      <ScreenHeader title="Top Viewed" description="Most viewed listings ranked by traffic" />
