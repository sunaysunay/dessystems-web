...
export async function GET(_req: NextRequest) {
  ...
  const [{ data: procs }, { data: edges }, { data: screens }, { data: allDeps }, { data: objs }] = await Promise.all([
    supabase.from('bop_objects').select('object_id, name, module').eq('type', 'process').order('name'),
  ...
  return NextResponse.json({
    processes: (procs ?? []).map((p: any) => ({ slug: p.object_id.replace('process:', ''), name: p.name, module: p.module ?? null })),
