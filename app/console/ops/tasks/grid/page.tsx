...
function BulkBar({ count, saving, onAction, onClear }: {
  count: number; saving: boolean;
  onAction: (action: string, payload?: Record<string, unknown>) => void;
  onClear: () => void;
}) {
  return (
    <div className="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 dark:bg-blue-950/40 dark:border-blue-800 px-4 py-2">
      <span className="text-xs font-bold text-blue-700 dark:text-blue-300">{count} selected</span>
      <button onClick={onClear} className="text-[10px] text-blue-500 hover:text-blue-700 underline">Clear</button>
      <div className="ml-auto flex gap-1.5 flex-wrap">
        <button disabled={saving} onClick={() => onAction('transition', { status: 'in_progress' })}
          className="rounded-lg bg-amber-100 px-2.5 py-1 text-[10px] font-semibold text-amber-700 hover:bg-amber-200 disabled:opacity-40">Start</button>
        <button disabled={saving} onClick={() => onAction('transition', { status: 'done' })}
          className="rounded-lg bg-emerald-100 px-2.5 py-1 text-[10px] font-semibold text-emerald-700 hover:bg-emerald-200 disabled:opacity-40">Done</button>
        <button disabled={saving} onClick={() => onAction('transition', { status: 'cancelled' })}
          className="rounded-lg bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-500 hover:bg-slate-200 disabled:opacity-40">Cancel</button>
        <span className="border-l border-blue-200 dark:border-blue-700 mx-0.5" />
        ...
        <span className="border-l border-blue-200 dark:border-blue-700 mx-0.5" />
        <button disabled={saving} onClick={() => { if (confirm(`Delete ${count} tasks?`)) onAction('delete'); }}
          className="rounded-lg bg-red-50 px-2.5 py-1 text-[10px] font-semibold text-red-600 hover:bg-red-100 disabled:opacity-40">Delete</button>
      </div>
    </div>
  );
}
...
export default function TaskGridPage() {
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      {selected.size > 0 && <BulkBar count={selected.size} saving={saving} onAction={(a, p) => void bulkAction(a, p)} onClear={() => setSelected(new Set())} />}
